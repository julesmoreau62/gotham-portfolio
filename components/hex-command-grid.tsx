...
function PriorityTile({
  ...
  return (
    ...
    >
      <div
        className={`relative h-full ${skipTransitions ? "" : "transition-all duration-700"} ${visible ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-8 scale-90"}`}
        style={skipTransitions ? undefined : { transitionDelay: `${200 + index * 120}ms` }}
      >
        ...
        {/* Main card */}
        <div
          className="relative h-full bg-card/70 backdrop-blur-xl border-2 rounded-xl p-3 md:p-4 overflow-hidden transition-all duration-300 active:scale-[0.98] active:brightness-125"
          style={{
...
function SecondaryTile({
  ...
  return (
    ...
    >
      <div
        className={`relative h-full ${skipTransitions ? "" : "transition-all duration-700"} ${visible ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-8 scale-90"}`}
        style={skipTransitions ? undefined : { transitionDelay: `${400 + index * 100}ms` }}
      >
        {/* Glow on hover */}
        <div
        ...
        {/* Card */}
        <div
          className="relative h-full bg-card/60 backdrop-blur-md border rounded-lg p-3 overflow-hidden transition-all duration-300 active:scale-[0.98] active:brightness-125"
          style={{
...
export function HexCommandGrid({ visible, skipTransitions = false }: { visible: boolean; skipTransitions?: boolean }) {
  ...
  return (
    <div
      ...
      className={`fixed inset-0 z-10 flex flex-col ${skipTransitions ? "" : "transition-all duration-1000"} ${visible ? "opacity-100" : "opacity-0 pointer-events-none"}`}
    >
      {/* Top HUD bar */}
      <header
        className={`h-10 md:h-11 flex items-center justify-between px-3 md:px-8 border-b border-border/50 bg-card/30 backdrop-blur-md z-50 shrink-0 ${skipTransitions ? "" : "transition-all duration-700"} ${visible ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"}`}
        style={skipTransitions ? undefined : { transitionDelay: "200ms" }}
      >
