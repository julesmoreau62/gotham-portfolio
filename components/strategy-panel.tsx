...
export function StrategyPanel({ open, onClose }: StrategyPanelProps) {
  ...
  useEffect(() => {
    ...
    // Skip intro on re-opens — go straight to briefing
              ...
              <SectionBlock title="STRATEGIC PIVOT: INDIA" icon={Globe} iconColor="text-[hsl(var(--neon-cyan))]" delay={200} ready={briefReady}>
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                  ...
                  <div className="lg:col-span-3 relative bg-card/50 border border-[hsl(var(--neon-cyan))]/15 rounded-lg overflow-hidden h-64 md:h-80">
                    ...
                    <div className="absolute top-[6%] left-[34%] hidden lg:block">
      ...
      <div className="flex items-end gap-2.5 h-24">
        {bars.map((bar, i) => (
          <div key={bar.label} className="flex-1 flex flex-col items-center h-full justify-end">
            <div
              className={`text-[7px] font-mono mb-1 transition-all duration-700 ease-out ${animate ? "opacity-100" : "opacity-0"}`}
              style={{ transitionDelay: `${600 + i * 150}ms`, color: i === 3 ? "hsl(var(--field-green))" : undefined }}
            >
              <span className={i < 3 ? "text-muted-foreground" : ""}>{bar.tooltip}</span>
            </div>
    ...
    <div className="relative">
      ...
      <div className="hidden md:flex items-center justify-center mb-4">
        <div className={`w-4 h-4 rounded-full ring-4 ring-background ${
          active ? "bg-[hsl(var(--neon-cyan))] shadow-[0_0_12px_hsl(186_100%_50%)]" : dimmed ? "bg-[hsl(var(--field-green))] shadow-[0_0_8px_hsl(160_84%_39%/0.4)]" : "bg-muted-foreground/30"
        }`} />
      </div>

      <div className={`bg-card/60 rounded-lg overflow-hidden transition-colors ${
        active ? "border border-[hsl(var(--neon-cyan))]/40 shadow-[0_0_20px_rgba(0,240,255,0.06)]" : dimmed ? "border border-dashed border-[hsl(var(--field-green))]/30" : "border border-border/40"
      }`}>
        {/* Header */}
        <div className={`p-3 border-b flex items-center justify-between ${
          active ? "bg-[hsl(var(--neon-cyan))]/5 border-[hsl(var(--neon-cyan))]/20" : dimmed ? "bg-[hsl(var(--field-green))]/5 border-[hsl(var(--field-green))]/15" : "bg-card/30 border-border/30"
        }`}>
          <div>
            <span className={`text-[8px] font-mono tracking-[0.3em] ${active ? "text-[hsl(var(--neon-cyan))]" : dimmed ? "text-[hsl(var(--field-green))]" : "text-muted-foreground"}`}>
              PHASE {phase}
            </span>
          ...
          {tag && (
            <span className={`text-[7px] font-mono font-bold px-1.5 py-0.5 rounded ${
              active ? "bg-[hsl(var(--neon-cyan))]/15 text-[hsl(var(--neon-cyan))]" : dimmed ? "bg-[hsl(var(--field-green))]/15 text-[hsl(var(--field-green))]" : "bg-secondary/50 text-muted-foreground"
            }`}>{tag}</span>
          )}
        </div>
          ...
          <div className="space-y-1 pt-1">
            {actions.map((a) => (
              <div key={a} className="flex items-center gap-1.5 text-[8px] font-mono">
                <ChevronRight className={`w-2.5 h-2.5 ${active ? "text-[hsl(var(--neon-cyan))]" : dimmed ? "text-[hsl(var(--field-green))]/60" : "text-muted-foreground/50"}`} />
                <span className={active ? "text-[hsl(var(--neon-cyan))]" : dimmed ? "text-[hsl(var(--field-green))]/80" : "text-muted-foreground"}>{a}</span>
              </div>
            ))}
          </div>
        </div>
